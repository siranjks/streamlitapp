...
def generate_row_hash(row):
    """Creates a unique ID for a row based on its core data to prevent duplicates."""
    data_string = f"{row['Trade_Date']}{row['Importer_Name']}{row['Exporter_Name']}{row['Raw_Description']}{row['Value_USD']}"
...
def main():
    ...
    if choice == "Upload Data":
        st.title("📥 Data Ingestion & Deduplication")
        st.write("Upload Tendata/TradeDataPro files. The system will automatically skip rows already in the database.")
        ...
        if files:
            for file in files:
                ...
                with st.expander("Link your columns to the Database"):
                ...
                if st.button(f"Process {file.name}"):
                    ...
                    # Deduplication Check
                    conn = sqlite3.connect(DB_NAME)
                    existing_hashes = pd.read_sql("SELECT Row_Hash FROM TradeData", conn)['Row_Hash'].tolist()
                    ...
                    for idx, row in df_std.iterrows():
                        ...
                        if row_hash in existing_hashes:
                            ...
                            if ai_resp and "|" in ai_resp:
                                trans, cat = ai_resp.split("|")
                                trans_val, cat_val = trans.strip(), cat.strip()
                            else:
                    ...
                    if new_rows:
                        ...
                        st.warning(f"All rows in {file.name} already exist in the database.")
                    conn.close()

    # --- PAGE 2: DASHBOARD & STRATEGY ---
        ...
        if df.empty:
            st.info("Database is empty. Please upload data.")
        else:
            # Filters
            ...
            with col2:
                st.plotly_chart(px.bar(filtered_df, x='Category', y='Value_USD', color='Exporter_Name', barmode='group', title="Category Value by Competitor"))

            st.divider()
            st.subheader("💡 Strategic Sales Assistant")
            strategy_query = st.text_input("Ask for a specific strategy (e.g., 'Targeting high-value buyers in Armenia')")
    ...
    # --- PAGE 3: ADMIN ---
        ...
        if st.button("Save Changes"):
            # When saving, we replace the whole table to ensure edits/deletes are finalized
